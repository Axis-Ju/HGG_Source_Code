...
for (i in 1:length(comparisons)) {
  ...
  if (nGroups == 2 & max(colSums(design)) == 1) {
    ## Cauchy test
    cauchyTest(psmData, "PSM", fdrMethod, saveDir, suffix, comparisons[i], comparisonNames[i], newColNames, compSamples)
    cauchyTest(pepData, "Peptide", fdrMethod, saveDir, suffix, comparisons[i], comparisonNames[i], newColNames, compSamples)
    if (exists("protData")) {
      cauchyTest(protData, "Protein", fdrMethod, saveDir, suffix, comparisons[i], comparisonNames[i], newColNames, compSamples)  
    }
    if (exists("siteData")) {
      cauchyTest(siteData, "Site", fdrMethod, saveDir, suffix, comparisons[i], comparisonNames[i], newColNames, compSamples)  
    }
